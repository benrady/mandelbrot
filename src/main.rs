...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair(",10", ','), none);
    assert_eq!(parse_pair("10,20", ','), pair);
    assert_eq!(parse_pair("10xy,20", ','), none);
    assert_eq!(parse_pair("10x20", 'x'), pair);
}

#[test]
fn test_no_escape() {
    let time = escape_time(Complex {re: 0.0, im: 0.0}, 1);
    assert_eq!(time, Option::None);
}
